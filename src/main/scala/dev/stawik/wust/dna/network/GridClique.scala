...
object GridClique {
  ...
  def gridQFactory[T <: Node](params: GridQParams, newNode: () => T): () => GridClique[T] = () => new GridClique(params, newNode)
}
class GridClique[T <: Node](params: GridQParams, newNode: () => T) extends Network[T](newNode){
  for(x <- 0 until params.sideA){
    for(y <- 0 until params.sideB) {
      for (n <- 0 until params.cliqueSize) {
        nodes += (GridQLoc(x, y, n) -> newNode())
      }
    }
  }
  for(x <- 0 until params.sideA) {
    for(y <- 0 until params.sideB) {
      ...
      for {i <- Seq((x - 1, 1), (x + 1, 3)) if i._1 >= 0 && i._1 < params.sideA
           j <- Seq((y - 1, 0), (y + 1, 2)) if j._1 >= 0 && j._1 < params.sideB
                     ...
                     nodes(GridQLoc(x,y, j._2)).receiveNeighbour(nodes(GridQLoc(x, j._1, (j._2 + 2) % 4))) }
